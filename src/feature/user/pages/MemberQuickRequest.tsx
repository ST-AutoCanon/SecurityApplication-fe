...
export default function MemberQuickRequest() {
  ...
  // ---------- AVAILABILITY ----------
  const loadAvailability = useCallback(async (requestTypeId: string | number, date: string) => {
    if (!date) {
      setSlotAvailability([]);
      return;
    }
    ...
    try {
      ...
      const res = await axios.get(
        `${API}/${requestTypeId}/availability?date=${date}`,
        { withCredentials: true }
      );
  ...
  const handleDateChange = (date: string) => {
    ...
    if (selectedType?.is_bookable) {
      loadAvailability(selectedType.id, date);
    }
  };
  ...
  const submitRequest = async () => {
    ...
    if (selectedType.is_bookable) {
      ...
      if (!selectedSlot) {
        setAlert({ type: "warning", message: "Please select a time slot" });
        return;
      }
    }
            ...
            {(showAllServices ? requestTypes : requestTypes.slice(0, 6)).map((type) => {
              ...
              return (
                ...
                >
                  ...
                  {type.is_bookable && (
                    <span className="mt-2 inline-flex w-fit rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-medium text-indigo-700">
                      Bookable
                    </span>
                  )}
                  <button
...
{selectedType.is_bookable && (
    ...
    {selectedDate && (
      <div>
        ...
        {loadingSlots ? (
          ...
          (() => {
            // Prefer live data, otherwise use the slots that came with the request type
            const slotsToShow =
              slotAvailability.length > 0
                ? slotAvailability
                : (selectedType.slots || []).map((s) => ({
                    slotId: s.id,
                    label: s.label,
                    status: "available",
                    available: true,
            ...
            if (slotsToShow.length === 0) {
              return (
                <p className="py-4 text-center text-sm text-rose-600">
                  No time slots found. Ask admin to add slots.
                </p>
              );
            }
                ...
                {slotsToShow.map((slot) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={isBooked}
                      onClick={() => !isBooked && setSelectedSlot(slot.slotId)}
                    ...
                    >
                      ...
                      <div className="mt-1.5">
                        {slot.status === "booked" && (
                          <span className="inline-flex rounded-full bg-rose-100 px-2.5 py-0.5 text-[11px] font-medium text-rose-700">
                            Booked
                          </span>
                        )}
                        {slot.status === "pending" && (
                          <span className="inline-flex rounded-full bg-amber-100 px-2.5 py-0.5 text-[11px] font-medium text-amber-800">
                            Chosen by someone – not yet approved
                          </span>
                        )}
                      ...
                      {slot.status === "pending" && (
                        <p className="mt-1.5 text-[11px] text-amber-700">
                          You can still request this slot. Admin will decide.
                        </p>
                      )}
                    </button>
                  );
  ...
  if (viewResponse.questions) {
    ...
    const type = requestTypes.find(
      (t) =>
        String(t.id) === String(viewResponse.requestTypeId) ||
        t.name === viewResponse.requestName
    );
    const q = type?.questions?.find((item) => String(item.id) === key);
    if (q) questionText = q.question;
  }

  // Nice labels for booking fields
  if (key === "date") questionText = "Booking Date";
  if (key === "slot") questionText = "Time Slot";

  // Resolve slot ID → label
  ...
  if (value != null && value !== "") {
    if (key === "date") {
      ...
      const slot = type?.slots?.find((s) => s.id === value);
      if (slot?.label) {
        displayValue = slot.label;
      } else {
        // Fallback: clean the id a bit
        const str = String(value);
        displayValue = str.replace(/_\d{10,}$/, "").replace(/_/g, " ") || str;
      }
  ...
  return (
    <div
      key={key}
      className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-3 px-3 py-2.5"
    >
      <span className="break-words text-sm font-medium text-slate-600">
        {questionText}
      </span>
      <span className="break-words text-sm text-slate-800">
        {displayValue}
              ...
              {(String(viewResponse.status || "").toLowerCase() === "rejected" ||
                String(viewResponse.status || "").toLowerCase() === "cancelled") &&
                viewResponse.rejectionReason && (
                  <div>
                    <p className="text-xs font-medium uppercase tracking-wide text-slate-400">
                      Cancellation Reason
                    </p>
              ...
              {(String(viewResponse.status || "").toLowerCase() === "approved" ||
                String(viewResponse.status || "").toLowerCase() === "completed") &&
                viewResponse.approvalComment && (
                  <div>
                    <p className="text-xs font-medium uppercase tracking-wide text-slate-400">
                      Approval Comment
                    </p>
