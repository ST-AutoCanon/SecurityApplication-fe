...
// function generateAnnouncementText(response: RequestResponse): string {
//   const answersText = response.answers
//     ? Object.entries(response.answers)
//         .map(([key, value]) => {
//           const q = response.questions?.find((item) => String(item.id) === key);
...
//   return [
//     `📢 Announcement – ${response.requestName}`,
//     ``,
//     `A request of type "${response.requestName}" was submitted by ${response.requestedBy} on ${response.date}.`,
//     `Status: ${response.status}`,
//     answersText ? `\nDetails:\n${answersText}` : "",
//     response.approvalComment ? `\nAdmin note: ${response.approvalComment}` : "",
//     ``,
...
//   const [responsesLoading, setResponsesLoading] = useState(false);
//   const [saving, setSaving] = useState(false);
// const [selectedTone, setSelectedTone] = useState<"formal" | "friendly" | "short" | "detailed">("formal");
//   // Alert
//   const [alert, setAlert] = useState<{
//     type: "success" | "warning" | "error";
//     message: string;
...
//   const [announcementResponse, setAnnouncementResponse] = useState<RequestResponse | null>(null);
...
//   // ---------- ANNOUNCEMENT ----------
//   // const openAnnouncementModal = (response: RequestResponse) => {
//   //   setAnnouncementResponse(response);
//   //   setAnnouncementText(
//   //     response.announcement?.trim()
//   //       ? response.announcement
//   //       : generateAnnouncementText(response)
//   //   );
//   //   setShowAnnouncementModal(true);
//   // };
// const openAnnouncementModal = async (
//   response: RequestResponse,
//   tone: "formal" | "friendly" | "short" | "detailed" = "formal"
// ) => {
//   setAnnouncementResponse(response);
...
//   try {
//     if (response.announcement?.trim()) {
//       setAnnouncementText(response.announcement);
//       return;
//     }

//     const res = await axios.post(
//       `${API}/responses/${response.id}/generate-announcement`,
//       { tone }, // send selected tone
//       { withCredentials: true }
//     );

//     if (res.data?.success && res.data.data?.announcement) {
//       setAnnouncementText(res.data.data.announcement);
//     } else {
//       setAnnouncementText(generateAnnouncementText(response));
//     }
//   } catch (err) {
...
//     setAnnouncementResponse(null);
//     setAnnouncementText("");
//   };

//   const saveAsAnnouncement = async () => {
//     if (!announcementResponse || !announcementText.trim()) return;

//     try {
//       setSavingAnnouncement(true);
//       await axios.patch(
//         `${API}/responses/${announcementResponse.id}/announcement`,
...
//       setAlert({ type: "success", message: "Announcement saved successfully" });
...
//                             <button
//                               onClick={() => openAnnouncementModal(response)}
...
//                             >
//                               <Bell size={13} />
//                               {response.announcement ? "Edit Announcement" : "Make as Announcement"}
//                             </button>
...
//               {showResponseDetails.announcement && (
//                 <div>
//                   <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Announcement</p>
//                   <pre className="mt-1 whitespace-pre-wrap rounded-xl border border-indigo-100 bg-indigo-50/50 p-3 text-sm text-indigo-900">
//                     {showResponseDetails.announcement}
//                   </pre>
//                 </div>
//               )}
...
//   ) : (
//     <div className="flex flex-col gap-3 sm:flex-row">
//       {/* Only show for Approved */}
//       {showResponseDetails.status === "Approved" && (
//         <button
//           onClick={() => {
//             setShowResponseDetails(null);
//             openAnnouncementModal(showResponseDetails);
//           }}
//           className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl border border-indigo-200 bg-indigo-50 py-2.5 text-sm font-medium text-indigo-700 hover:bg-indigo-100"
//         >
//           <Bell size={16} />
//           {showResponseDetails.announcement ? "Edit Announcement" : "Make as Announcement"}
//         </button>
//       )}
...
//       {/* ===== Make as Announcement Modal ===== */}
//       {/* {showAnnouncementModal && announcementResponse && (
//         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
//           <div className="w-full max-w-xl overflow-hidden rounded-2xl bg-white shadow-xl">
//             <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
//               <div>
//                 <h3 className="font-semibold text-slate-900">Make as Announcement</h3>
//                 <p className="mt-0.5 text-xs text-slate-500">
//                   {announcementResponse.requestName} · {announcementResponse.requestedBy}
//                 </p>
//               </div>
//               <button
...
//                 Announcement text (you can edit)
//               </label>
//               <textarea
//                 value={announcementText}
//                 onChange={(e) => setAnnouncementText(e.target.value)}
//                 rows={12}
//                 className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-relaxed outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
//                 placeholder="Announcement content..."
//               />
//               <p className="mt-2 text-xs text-slate-500">
//                 This text will be saved on the response and can be shown to users.
//               </p>
//             </div>
...
//               </button>
//               <button
//                 onClick={saveAsAnnouncement}
//                 disabled={savingAnnouncement || !announcementText.trim()}
//                 className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-indigo-600 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-60"
//               >
//                 {savingAnnouncement ? (
...
//         <div>
//           <h3 className="text-lg font-semibold text-slate-900">Make as Announcement</h3>
//           <p className="mt-0.5 text-sm text-slate-500">
//             {announcementResponse.requestName} · {announcementResponse.requestedBy}
//           </p>
//         </div>
...
//                   openAnnouncementModal(announcementResponse, tone.value as any);
//                 }}
...
//             </label>
//             <span className="text-xs text-slate-400">You can edit it</span>
//           </div>
...
//               <p className="text-sm font-medium text-slate-700">Generating with AI...</p>
//             </div>
//           ) : (
//             <textarea
//               value={announcementText}
//               onChange={(e) => setAnnouncementText(e.target.value)}
//               rows={11}
//               className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-relaxed outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
//               placeholder="Announcement will appear here..."
//             />
//           )}
//         </div>
//       </div>
...
//         >
//           {savingAnnouncement ? <Loader2 size={16} className="animate-spin" /> : <Bell size={16} />}
//           Save Announcement
//         </button>
//       </div>
//     </div>
...
function generateAnnouncementText(
  response: RequestResponse,
  allRequests: QuickRequestItem[] = []
): string {
  const answersText = response.answers
    ? Object.entries(response.answers)
        .filter(([key]) => key !== "slotLabel") // hide internal field if present
        .map(([key, value]) => {
          // Label
          let label = key;
          if (key === "date") label = "Booking Date";
          else if (key === "slot") label = "Time Slot";
          else {
            const q = response.questions?.find((item) => String(item.id) === key);
          ...
          if (key === "date" && value) {
            try {
              displayValue = new Date(String(value)).toLocaleDateString("en-GB", {
                day: "2-digit",
                month: "short",
                year: "numeric",
              });
            ...
            // Prefer stored label
            if (response.answers?.slotLabel) {
              displayValue = response.answers.slotLabel;
            } else {
              // Resolve from request type slots
              const requestType =
                allRequests.find((r) => String(r.id) === String(response.requestTypeId)) ||
                allRequests.find((r) => r.name === response.requestName);

              const slot = requestType?.slots?.find((s) => s.id === value);
              if (slot?.label) {
                displayValue = slot.label;
              } else {
                // Fallback: clean the id
                const str = String(value);
                displayValue =
                  str.replace(/_\d{10,}$/, "").replace(/_/g, " ") || str;
              }
            }
    ...
    try {
      ...
      setRequests(
        data.map((item: any) => ({
          ...
          max_bookings_per_slot: Number(item.max_bookings_per_slot) || 1,
          questions: (item.questions || []).map((q: any) => ({
            id: q.id,
            question: q.question,
            type: (q.type as QuestionType) || "text",
...
function formatAnswerValue(
  ...
  if (key === "slot") {
    // Try to find the label from the request type's slots
    const requestType =
      allRequests.find((r) => String(r.id) === String(response.requestTypeId)) ||
      allRequests.find((r) => r.name === response.requestName);

    if (requestType?.slots?.length) {
      const slot = requestType.slots.find((s) => s.id === value);
      if (slot?.label) return slot.label;
    }

    // Fallback: clean the id a bit if label not found
    const str = String(value);
    const withoutTs = str.replace(/_\d{10,}$/, ""); // remove _1789930789662
    return withoutTs.replace(/_/g, " ") || str;
  }
...
function formatAnswerLabel(
  ...
  if (key === "date") return "Booking Date";
  if (key === "slot") return "Time Slot";

  const question = response.questions?.find((item) => String(item.id) === key);
  ...
  const editRequest = (request: QuickRequestItem) => {
    ...
    setIcon(request.icon || "other");
    setQuestions(request.questions);
    setIsBookable(Boolean(request.is_bookable));
    setSlots(request.slots || []);
    setMaxPerSlot(request.max_bookings_per_slot || 1);
    setStep(1);
    setActiveSection("create");
    setShowMenu(null);
  };
    ...
    try {
      ...
      await axios.post(
        API,
        {
          ...
          status: "Active",
          icon: request.icon || "other",
          is_bookable: request.is_bookable || false,
          slots: request.slots || [],
  ...
  const addSlot = () => {
    ...
    const id = newSlotLabel.trim().toLowerCase().replace(/\s+/g, "_") + "_" + Date.now();
    setSlots((prev) => [...prev, { id, label: newSlotLabel.trim() }]);
    setNewSlotLabel("");
  };
  ...
  const saveRequest = async () => {
    ...
    if (isBookable && slots.length === 0) {
      setAlert({ type: "warning", message: "Please add at least one time slot" });
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        slots: isBookable ? slots : [],
        max_bookings_per_slot: isBookable ? maxPerSlot : 1,
        questions: questions.map((q) => ({
          question: q.question,
          type: q.type,
          required: q.required,
  ...
  // ---------- ANNOUNCEMENT ----------
  const openAnnouncementModal = async (response: RequestResponse) => {
  setAnnouncementResponse(response);
  setShowAnnouncementModal(true);
  setAnnouncementText("");
  setSavingAnnouncement(true);
  ...
  try {
    const res = await axios.post(
      `${API}/responses/${response.id}/generate-announcement`,
      { tone: "normal" }, // or omit tone if backend defaults to normal
      { withCredentials: true }
    );
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <div>
                      <p className="text-sm font-medium text-slate-800">Enable Date & Slot Booking</p>
                      <p className="text-xs text-slate-500">
                        For hall booking, parking, etc. Members will pick a date and time slot.
                      </p>
                    </div>
                  </label>
                    ...
                    <div className="mt-4 space-y-4 border-t border-slate-200 pt-4">
                      <div>
                        ...
                        <input
                          ...
                          min={1}
                          value={maxPerSlot}
                          onChange={(e) => setMaxPerSlot(Math.max(1, Number(e.target.value) || 1))}
                          className="w-32 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-400"
                        />
                      </div>
                  ...
                  <div>
                    ...
                    {isBookable && (
                      <span className="mt-1 inline-flex rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-medium text-indigo-700">
                        Bookable · {slots.length} slots · Max {maxPerSlot}/slot
                      </span>
                    )}
                  </div>
                </div>
      ...
      {showPreview && selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white shadow-xl">
            ...
            <div className="p-5">
              <p className="mb-5 text-sm text-slate-600">{selectedRequest.description}</p>
              {selectedRequest.is_bookable && (
                <div className="mb-4 rounded-lg bg-indigo-50 px-3 py-2 text-xs text-indigo-700">
                  Bookable · {selectedRequest.slots?.length || 0} slots · Max {selectedRequest.max_bookings_per_slot || 1} per slot
                </div>
              )}
              <div className="space-y-4">
                {selectedRequest.questions.map((q) => (
  ...
  <div key={key} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-3 px-3 py-2.5">
    <span className="break-words text-sm font-medium text-slate-600">
      {formatAnswerLabel(key, showResponseDetails)}
    </span>
    <span className="break-words text-sm text-slate-800">
      {formatAnswerValue(key, value, showResponseDetails, requests)}
    </span>
  </div>
))}
                    </div>
